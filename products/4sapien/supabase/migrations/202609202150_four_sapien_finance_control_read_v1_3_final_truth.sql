-- v1.3 canonical Money Now: merge v1.2 overdue/NULL safeguards with no-double-today filtering.
-- v1.2: unconfirmed overdue bills invalidate a spendable-money projection.
-- Source truth preserved; no automatic paid/unpaid inference or writes.
...
begin
  ...
  with eligible as (
    ...
    where e.user_id=v_uid and e.state in ('active','upcoming')
      ...
      and not (e.type='bill' and coalesce(e.meta->>'payment_state','')='PAID')
      -- A once-only spend or income already dated today is not an upcoming cashflow.
      -- Unpaid upcoming bills and recurring schedules remain models, not payment proof.
  ...
  where e.user_id=v_uid and e.type='bill' and e.state in ('active','upcoming')
    ...
    and (e.state='upcoming' or e.meta->>'payment_state' in ('NOT_MARKED_PAID','UNPAID'))
    and not (lower(e.source) in ('import','imported')
      and lower(e.truth) not in ('confirmed','user_confirmed','source_verified')
      and coalesce((e.meta->>'confirmed')::boolean,false)=false);
  v_anticipated_balance:=case when v_status='KNOWN_RECORDED' and jsonb_array_length(v_overdue)=0
  ...
  return jsonb_build_object(
    ...
    'before_next_income',jsonb_build_object(
      'state',case when jsonb_array_length(v_overdue)>0 then 'UNRESOLVED_OVERDUE'
        when v_next_income is null then 'NEXT_INCOME_UNKNOWN'
        when v_status<>'KNOWN_RECORDED' then v_status
        else 'MODELLED_NOT_AVAILABLE_CASH' end,
      'projected_without_income',case when v_status='KNOWN_RECORDED' and v_next_income is not null and jsonb_array_length(v_overdue)=0 then
    ...
    'until_horizon',jsonb_build_object(
      ...
      'of_which_recurring_modelled',v_recurring_outgoings,'truth',case when jsonb_array_length(v_overdue)>0 then 'MODELLED_INCOMPLETE_UNRESOLVED_OVERDUE' else 'MODELLED_NOT_BANK_BALANCE' end),
    'data_quality',jsonb_build_object(
      'pending_csv_review_batches',v_pending_count,
      'overdue_unconfirmed_count',jsonb_array_length(v_overdue),'has_bank_connection',false,
