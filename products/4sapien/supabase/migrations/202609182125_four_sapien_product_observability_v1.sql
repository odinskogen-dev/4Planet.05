-- 4SAPIEN ABSOLUTE LIVE PRODUCT CLOSURE + REAL USER READINESS 02
-- Observability is derived from the existing event stream only.
-- No new truth store, no copied user data, no destructive rewrites.
...
select
  ...
  bool_or(event_type = 'finance_opened') as finance_used
