...
Pass: current programme meaning remains on the existing `CURRENT_STATE_REV` when Founder authority, programme allocation, gates, acceptance semantics, blocker meaning and action safety are unchanged. This unchanged-semantics classification is `RUNTIME_ONLY`. The moving machine state is refreshed as a timestamped volatile observation with exact ref/value/SHA, source, freshness, materiality and revalidation requirement.
