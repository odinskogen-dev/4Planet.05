...
test('standalone ecosystem card carries corridor, effort, cetacean context, pressure, monitoring and action seams', () => {
  ...
  // Funding is for monitorable delivery, not ecological promises. Keep the
  // current human-facing sentence flexible while locking the truth boundary.
  assert.match(ecosystemHtml, /Delivery evidence must remain separate from ecological outcome claims/i);
  assert.doesNotMatch(ecosystemHtml, /guarantee(?:d|s)? ecological outcome/i);
  assert.doesNotMatch(ecosystemHtml, /promise(?:d|s)? ecological outcome/i);
});
