...
test('TEST KING Jaguar 33 uses the controlled local Ear-derived triangle surface, physical jungle room, LUME room and all eight chapters',()=>{
  ...
  assert.match(html,/jaguar-surface-v33\.js/);assert.match(html,/jaguar-surface-repair-v35\.js/);assert.match(html,/jaguar-gold-v33\.js/);assert.match(html,/jaguar-gold-v33\.css/);
  assert.doesNotMatch(html,/jaguar-ear-bridge-v34\.js/);assert.doesNotMatch(html,/jaguar-data-(meta|pos|nrm|col)-v29\.js/);assert.doesNotMatch(html,/jaguar-gold-v32\.js/);assert.doesNotMatch(html,/<iframe/i);
  ...
  assert.match(html,/RECONSTRUCTED TOPOLOGY · NOT LIVE TRACKING/);assert.match(html,/not the original textured or rigged source model/);
  assert.match(bridge,/91c61c329d2a4668816f81f08dfcd492/);assert.match(bridge,/viewerready/);assert.match(bridge,/getSceneGraph/);
