...
test('TEST KING Jaguar uses controlled Ear.Rodriguez indexed donor topology, deterministic camera fit, physical jungle room, LUME room and all eight chapters',()=>{
  ...
  assert.match(calibration,/CONTROLLED_DONOR_INDEX_BUFFER/);assert.match(calibration,/DYNAMIC_BOUNDS_NORMALISATION/);assert.match(calibration,/targetLongestSpan=3\.45/);assert.match(calibration,/targetCentre=\[0,1\.58,0\]/);assert.match(calibration,/targetLongestSpan\/sourceMaxSpan/);assert.match(calibration,/WITHOUT_MUTATING_SOURCE_TOPOLOGY_OR_PAYLOAD/);assert.doesNotMatch(calibration,/magnification=18/);assert.doesNotMatch(calibration,/__JAGS33_I\s*=/);assert.doesNotMatch(calibration,/__JAGS33_P\s*=/);
