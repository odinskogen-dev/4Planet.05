...
test('TEST KING Jaguar uses verified Ear.Rodriguez full-source viewer with controlled indexed fallback, canonical jungle/LUME room and all eight chapters',()=>{
  ...
  assert.match(full,/91c61c329d2a4668816f81f08dfcd492/);assert.match(full,/sketchfab\.com\/models\/\$\{MODEL_UID\}\/embed/);assert.match(full,/direct-official-embed-v47/);assert.match(full,/jaguarEarFull = 'loading'/);assert.match(full,/jaguarEarFull = 'ready'/);assert.match(full,/jaguarEarFull = 'fallback'/);assert.match(full,/direct-embed-timeout/);assert.match(full,/animation_autoplay=1/);assert.match(full,/visibilitychange/);assert.match(full,/about:blank/);assert.match(full,/CC BY 4\.0/);
  assert.match(full,/ensureShellMounted/);assert.match(full,/shell\.isConnected/);assert.match(full,/stage\.appendChild\(shell\)/);assert.match(full,/enter\?\.addEventListener\('click'/);
  assert.match(full,/function assertFullSourceOwnership/);assert.ok((full.match(/assertFullSourceOwnership\(\)/g)||[]).length>=3,'full-source ownership must be asserted on definition, initial ready and resumed/ENTER-ready path');
