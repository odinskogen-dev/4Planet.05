...
const staticRoutes = [
  "/", "/domains", "/missions", "/living-systems", "/atlas", "/species", "/impact",
  "/magazine", "/magazine/about", "/magazine/sources", "/magazine/corrections", "/magazine/archive",
  "/about", "/privacy", "/join",
];
...
const sitemapUrls = routes.map((route) => `  <url><loc>${escapeXml(`${origin}${route}`)}</loc></url>`).join("\n");
const sitemap = `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${sitemapUrls}\n</urlset>\n`;
fs.writeFileSync(path.join(publicDir, "sitemap.xml"), sitemap, "utf8");

// News sitemap remains deliberately strict: only full stories with a real
// publication timestamp inside the last 48 hours enter Google News XML.
const now = Date.now();
...
console.log(`Generated sitemap.xml with ${routes.length} URLs, news-sitemap.xml with ${newsStories.length} fresh stories, rss.xml with ${stories.length} stories + ${signals.length} signals for ${origin}`);
