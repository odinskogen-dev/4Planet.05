...
if (!fs.existsSync(indexPath)) throw new Error("dist/index.html is missing; run Vite build first");
if (!fs.existsSync(manifestPath)) throw new Error("src/content/shareMeta.json is missing");
...
function headMarkup(route, meta) {
  const canonical = absoluteUrl(meta.canonicalPath || route);
  ...
  const jsonLd = JSON.stringify({
    ...
    url: canonical,
    primaryImageOfPage: { "@type": "ImageObject", url: image, caption: meta.imageAlt },
    isPartOf: { "@type": "WebSite", name: manifest.siteName, url: absoluteUrl("/") },
  }).replaceAll("<", "\\u003c");
...
function writeRoute(route, meta) {
  ...
  const target = route === "/" ? indexPath : path.join(dist, route.replace(/^\//, ""), "index.html");
  fs.mkdirSync(path.dirname(target), { recursive: true });
  fs.writeFileSync(target, html, "utf8");
}

for (const [route, meta] of Object.entries(manifest.routes)) writeRoute(route, meta);

console.log(`Prerendered ${Object.keys(manifest.routes).length} restrained 4PLANET share fronts at ${origin}`);
