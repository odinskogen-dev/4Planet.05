...
const jaguarBridge = read('public/xr/engine/nature-jaguar-sketchfab-v23.js');
...
test('Jaguar master is one eight-frame end-to-end Gold Journey with truthful action boundaries', () => {
  ...
  assert.match(jaguarHtml, /nature-jaguar-sketchfab-v23\.js/);
  assert.doesNotMatch(jaguarHtml, /nature-jaguar-local-v26\.js|jaguar-ear-proxy-v25\.js/);
  assert.match(jaguarHtml, /jaguar-master-v26\.css/);
  assert.match(jaguarBridge, /91c61c329d2a4668816f81f08dfcd492/);
  assert.match(jaguarBridge, /identityScene/);
