...
test('Jaguar master v26 protects smooth runtime, local 3D presence and the authored encounter room', () => {
  ...
  const renderer = read('public/xr/engine/nature-jaguar-local-v26.js');
  ...
  assert.match(index, /jaguar-master-v26\.css/);
  assert.match(index, /nature-runtime-budget-v21\.js/);
  assert.match(index, /nature-jaguar-local-v26\.js/);
  ...
  assert.match(renderer, /Math\.min\(dpr, 1\.2\)/);
  assert.match(renderer, /Math\.min\(dpr, 1\)/);
  assert.match(renderer, /Math\.min\(dpr, \.82\)/);
  assert.match(renderer, /document\.hidden/);
  assert.match(renderer, /pagehide/);
  assert.match(renderer, /runtimeBudget\(\) === 'lite' \? 28 : 58|runtimeBudget\(\) === 'lite' \? 18 : 54/);
  ...
  assert.match(localCss, /data-cinematic-scene="dependency"[\s\S]*display:none!important/);
  assert.match(localCss, /data-cinematic-scene="proof"/);
});
