...
test("four public products have distinct routes and a context-preserving switcher", () => {
  ...
  assert.match(routes, /function isLabsHost\\(\\)/);
  assert.match(routes, /labs\\.4planet\\.org/);
  assert.match(routes, /return isLabsHost\\(\\) \\? <LabsV4 \\/> : <Home \\/>;/);
  assert.match(routes, /<Route path="\\*" element=\\{isLabsHost\\(\\) \\? <LabsV4 \\/> : <NotFound \\/>\\}/);
