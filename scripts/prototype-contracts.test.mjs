...
test("four public products have distinct routes and a context-preserving switcher", () => {
  ...
  assert.ok(routes.includes('<Route path="/" element={<RootHome />} />'));
  assert.ok(routes.includes('function isLabsHost()'));
  assert.ok(routes.includes('labs.4planet.org'));
  assert.ok(routes.includes('return isLabsHost() ? <LabsV4 /> : <Home />;'));
  assert.ok(routes.includes('<Route path="*" element={isLabsHost() ? <LabsV4 /> : <NotFound />} />'));
