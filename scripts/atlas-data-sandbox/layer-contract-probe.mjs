...
const checks = [
  ...
  { id: "emodnet-habitat", label: "Seabed habitats 2023", kind: "image", expected: "OPEN_LAB", url: wms3857("https://ows.emodnet-seabedhabitats.eu/geoserver/emodnet_view/wms", "eusm2023_eunis2019_group", { style: "default-style-eusm2023_eunis2019_group", version: "1.3.0" }) },
...
async function run(check) {
  ...
  try {
    ...
    if (!response.ok) {
      const text = (await response.text()).slice(0, 500).replace(/\s+/g, " ");
      return { ...check, state: "SOURCE_OR_CONTRACT_DOWN", httpStatus: response.status, contentType, errorSnippet: text, durationMs: Math.round(performance.now() - started) };
    }
    if (check.kind === "image") {
      const bytes = await response.arrayBuffer();
      const good = contentType.toLowerCase().includes("image") && bytes.byteLength >= 100;
      const errorSnippet = good ? undefined : new TextDecoder().decode(bytes.slice(0, 500)).replace(/\s+/g, " ");
      return { ...check, state: good ? "LAYER_CONTRACT_GREEN" : "CONTRACT_MISMATCH", httpStatus: response.status, contentType, bytes: bytes.byteLength, errorSnippet, durationMs: Math.round(performance.now() - started) };
    }
...
for (const check of checks) {
  ...
  results.push(result);
  console.log(`${result.state.padEnd(28)} ${result.id.padEnd(31)} ${String(result.httpStatus ?? "-").padStart(3)} ${String(result.durationMs).padStart(6)}ms${result.errorSnippet ? ` :: ${result.errorSnippet.slice(0, 180)}` : ""}`);
}
...
const md = [
  ...
  "",
  "This checks concrete tile/record requests shaped like ATLAS requests, not only the provider homepage/API root. LAYER_CONTRACT_GREEN still requires browser-visible proof before MAP_GREEN.",
  "",
  "| Layer | Contract | HTTP | Evidence |",
  "|---|---|---:|---|",
  ...clean.map((r) => `| ${r.label} | ${r.state} | ${r.httpStatus ?? "—"} | ${r.bytes ? `${r.bytes} image bytes` : Number.isFinite(r.count) ? `${r.count} records` : r.errorSnippet || r.contentType || "—"} |`),
  "",
  "## Counts",
