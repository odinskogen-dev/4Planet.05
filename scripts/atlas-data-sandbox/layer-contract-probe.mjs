...
// z7/x66/y39: bounded North Sea MapLibre tile. The broad habitat product is
// expensive at continental z3 scale; this probe must reflect a real map tile,
// not a giant synthesis request that can time out while the tiled map is usable.
const NORTH_SEA_TILE_Z7_BBOX_3857 = "626172.1357121654,7514065.628545966,939258.2035682462,7827151.696402049";
...
const checks = [
  ...
  { id: "emodnet-habitat", label: "Seabed habitats 2025 · MSFD 800 m · North Sea map tile", kind: "image", expected: "OPEN_LAB", url: wms3857("https://ows.emodnet-seabedhabitats.eu/geoserver/emodnet_view/wms", "eusm2025_msfd_800", { style: "eusm2019_msfd_800", version: "1.3.0", bbox: NORTH_SEA_TILE_Z7_BBOX_3857 }) },
  ...
  { id: "noaa-coral-dhw", label: "Coral heat stress · latest", kind: "image", expected: "OPEN_REPAIRED", url: "https://coastwatch.pfeg.noaa.gov/erddap/wms/NOAA_DHW/request?service=WMS&version=1.3.0&request=GetMap&layers=NOAA_DHW%3ACRW_DHW&styles=&format=image%2Fpng&transparent=true&crs=CRS%3A84&width=512&height=256&time=current&bbox=-180,-35,180,35" },
...
const report = { schemaVersion: 9, generatedAt: new Date().toISOString(), commitSha: process.env.GITHUB_SHA || null, counts, results: clean, note: "Seabed habitat uses a bounded z7 North Sea MapLibre tile contract rather than a continental synthesis request. NOAA Coral Reef Watch uses the current official NOAA_DHW / CRW_DHW WMS contract. Byte-level image transport still requires browser-visible proof before MAP_GREEN." };
const md = ["# ATLAS DATA LAB — EXACT LAYER CONTRACT REPORT", "", `Generated: ${report.generatedAt}`, `Commit: ${report.commitSha || "local"}`, "", "This checks concrete tile/record requests shaped like ATLAS requests, not only the provider homepage/API root. LAYER_CONTRACT_GREEN still requires browser-visible proof before MAP_GREEN. The seabed-habitat request is a bounded z7 North Sea MapLibre tile, matching real tiled-map scale instead of a continental synthesis request. NOAA Coral Reef Watch is bound to the current official NOAA_DHW / CRW_DHW WMS product.", "", "| Layer | Contract | HTTP | Evidence |", "|---|---|---:|---|", ...clean.map((r) => `| ${r.label} | ${r.state} | ${r.httpStatus ?? "—"} | ${r.bytes ? `${r.bytes} image bytes` : Number.isFinite(r.count) ? `${r.count} records` : r.errorSnippet || r.contentType || "—"} |`), "", "## Counts", ...Object.entries(counts).map(([state, count]) => `- ${state}: ${count}`), ""].join("\n");
