...
test('TEST KING Jaguar 33 renders Ear surface 3D, exposes LUME room, and completes 8 scenes', async ({ page }) => {
  ...
  await expect(root).toHaveAttribute('data-jaguar-visual','visible',{timeout:5000});
  const pixelProof=await root.evaluate(el=>({opaque:Number(el.getAttribute('data-jaguar-pixel-ratio')||0),lit:Number(el.getAttribute('data-jaguar-lit-ratio')||0)}));expect(pixelProof.opaque).toBeGreaterThan(.002);expect(pixelProof.lit).toBeGreaterThan(.001);
