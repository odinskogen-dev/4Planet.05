...
for (const project of ["desktop-1440", "mobile-390"] as const) {
  test.describe(`${project} Jaguar Gold Reference`, () => {
    test.skip(({ }, testInfo) => testInfo.project.name !== project, `Run only in ${project}`);

    test("species world exposes Atlas, living web, ecosystem and motion without breaking truth boundaries", async ({ page }, testInfo) => {
