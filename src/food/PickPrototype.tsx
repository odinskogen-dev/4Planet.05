...
function EmptyState() { return <section className="pick-empty"><span className="pick-kicker">PRIVATE PROTOTYPE · NORWAY</span><h2>One product.<br />Three truths.</h2><p>Scan or enter a barcode. PICK_ keeps HEALTH, WALLET and PLANET separate and shows unknowns instead of inventing certainty.</p></section>; }
...
export default function PickPrototype() {
  ...
  const readProduct = async (value = gtin) => {
    ...
    if (!parsed.ok) { setState("malformed"); setMessage(parsed.error === "invalid_check_digit" ? "Invalid GTIN check digit." : "Use an 8, 12, 13 or 14 digit GTIN."); setResult(null); return; }
    ...
    } catch (error) { setResult(null); setState("source_error"); setMessage(error instanceof Error ? error.message : "Source request failed."); }
  };
  ...
  const addCurrent = () => { if (product) saveBasket(addBasketItem(basket, makeBasketItem(product, axes))); };
  const removeCurrent = () => { if (product) saveBasket(removeBasketItem(basket, product.gtin)); };

  return <main className="pick-shell">
    <header className="pick-header"><Link to="/" className="pick-wordmark" aria-label="4PLANET home">4PLANET_</Link><div className="pick-header__product"><strong>PICK_</strong><span>FOOD INTELLIGENCE · PRIVATE</span></div><a href="#basket" className="pick-header__link">BASKET {basket.length}</a></header>
    ...
    <section className="pick-scan" aria-label="Product lookup"><form onSubmit={submit}><label htmlFor="pick-gtin">BARCODE / GTIN</label><div className="pick-input-row"><input id="pick-gtin" inputMode="numeric" autoComplete="off" value={gtin} onChange={(event) => setGtin(event.target.value.replace(/\D/g, ""))} aria-describedby="pick-input-help" /><button type="submit" disabled={state === "loading"}>{state === "loading" ? "READING" : "READ PRODUCT"}</button></div><p id="pick-input-help">Use the existing camera scanner for shelf testing, or enter a barcode manually here.</p></form><Link className="pick-camera-cta" to="/labs/food-intelligence">OPEN CAMERA SCANNER →</Link></section>
