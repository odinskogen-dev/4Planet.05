...
function fmt(value: number | null, unit = "g") { return value === null ? "—" : `${value.toLocaleString("nb-NO", { maximumFractionDigits: 1 })} ${unit}`; }

function AxisCard({ axis }: { axis: ReturnType<typeof buildDecisionAxes>[number] }) {
  return <article className="pick-axis" data-axis={axis.id} data-confidence={axis.confidence.toLowerCase()}><div className="pick-axis__head"><span className="pick-kicker">{axis.label}</span><span className="pick-confidence">{axis.confidence}</span></div><strong>{axis.state}</strong><p>{axis.summary}</p><small>{axis.directness} · {axis.limitation}</small></article>;
}

function EmptyState({ onDetected }: { onDetected: (gtin: string) => void }) {
  return <section className="pick-empty pick-empty--v2"><div><span className="pick-kicker">PRIVATE PROTOTYPE · NORWAY</span><h2>Scan.<br />Know.<br /><i>Pick.</i></h2><p>One product decision across HEALTH, WALLET and PLANET, with uncertainty kept visible.</p></div><PickScanner onDetected={onDetected} /></section>;
}
...
export default function PickPrototype() {
  ...
  const evidenceSources = passport.evidenceSources as Array<{ id?: string; sourceId?: string; title?: string; sourceClass?: string; class?: string; url?: string; checkedAt?: string }>;

  const readProduct = async (value = gtin) => {
    const parsed = normalizeGtin(value);
    if (!parsed.ok) { setState("malformed"); setMessage(parsed.error === "invalid_check_digit" ? "Invalid GTIN check digit." : "Use an 8, 12, 13 or 14 digit GTIN."); setResult(null); setWallet(unknownWallet()); return; }
    ...
    try {
      ...
      setWallet(priceRead.status === "fulfilled" ? normaliseWalletEnvelope(priceRead.value as Record<string, any>) : unknownWallet("Price source could not be read."));
    } catch (error) {
      setResult(null); setState("source_error"); setWallet(unknownWallet("Price context unavailable.")); setMessage(error instanceof Error ? error.message : "Source request failed.");
    }
  };
  ...
  return <main className="pick-shell pick-shell--v2" data-shop-mode={shopMode ? "yes" : "no"}>
    <header className="pick-header"><Link to="/" className="pick-wordmark" aria-label="4PLANET home">4PLANET_</Link><div className="pick-header__product"><strong>PICK_</strong><span>FOOD INTELLIGENCE · PRIVATE</span></div><div className="pick-header-actions"><button type="button" className="pick-mode-toggle" aria-pressed={shopMode} onClick={() => setShopMode((value) => !value)}>{shopMode ? "SHOP MODE" : "EXPLORE MODE"}</button><a href="#basket" className="pick-header__link">BASKET {basket.length}</a></div></header>
    ...
    <section className="pick-scan pick-scan--v2" aria-label="Product lookup"><PickScanner onDetected={detected} /><form onSubmit={submit}><label htmlFor="pick-gtin">BARCODE / GTIN</label><div className="pick-input-row"><input id="pick-gtin" inputMode="numeric" autoComplete="off" value={gtin} onChange={(event) => setGtin(event.target.value.replace(/\D/g, ""))} /><button type="submit" disabled={state === "loading"}>{state === "loading" ? "READING" : "READ PRODUCT"}</button></div><p>Manual fallback remains available when camera detection is unsupported.</p></form></section>
    ...
    {state === "idle" && <EmptyState onDetected={detected} />}
    ...
    {state === "found" && product && <>
      ...
      <section className="pick-truth-spine" aria-labelledby="pick-truth-title"><div className="pick-section-head"><div><span className="pick-kicker">WHY / BASED ON WHAT</span><h2 id="pick-truth-title">Truth spine</h2></div><span>Inspect the chain before trusting the conclusion.</span></div><div className="pick-chain" aria-label="Truth chain">{passport.chain.map((step, index) => <div key={step}><span>{String(index + 1).padStart(2,"0")}</span><strong>{step}</strong>{index < passport.chain.length - 1 && <i aria-hidden="true">→</i>}</div>)}</div><div className="pick-passport-grid"><article><span>PRODUCT SOURCE</span><strong>{passport.source.id}</strong><p>{passport.source.class}</p><small>API {passport.source.apiVersion ?? "UNKNOWN"} · Licence: {passport.source.licence}</small></article><article><span>DIRECTNESS</span><strong>{passport.directness}</strong><p>Exact product facts stay separate from category/pattern evidence.</p></article><article><span>FRESHNESS</span><strong>{passport.freshness.state}</strong><p>{passport.freshness.detail}</p></article><article><span>CONFLICT STATE</span><strong>{passport.conflictState}</strong><p>{passport.completeness}% controlled field completeness.</p></article></div><details className="pick-evidence-details"><summary>SHOW FIELD EVIDENCE</summary><div className="pick-evidence-list">{passport.facts.map((fact) => <div key={fact.id} data-available={fact.available ? "yes" : "no"}><span>{fact.available ? "KNOWN" : "UNKNOWN"}</span><strong>{fact.label}</strong><small>{fact.directness} · {fact.interpretation}</small></div>)}</div></details>{evidenceSources.length > 0 && <div className="pick-source-ledger"><span className="pick-kicker">EVIDENCE SOURCES USED</span>{evidenceSources.map((source, index) => <article key={`${source.id ?? source.sourceId ?? "source"}-${index}`}><strong>{source.title ?? source.id ?? source.sourceId ?? "Source"}</strong><span>{source.sourceClass ?? source.class ?? "SOURCE"}{source.checkedAt ? ` · checked ${source.checkedAt}` : ""}</span>{source.url ? <a href={source.url} target="_blank" rel="noreferrer">OPEN SOURCE ↗</a> : null}</article>)}</div>}</section>
