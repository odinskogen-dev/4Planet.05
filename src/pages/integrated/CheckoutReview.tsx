    ...
    <PublicShell>
      ...
      <Section pad="clamp(48px,7vw,90px)">
        ...
        {offer && <div style={{ marginTop: 42, maxWidth: 820 }}>
          ...
          <div style={box}><div style={mono}>WHAT</div><div style={{ fontSize: 25, marginTop: 9, color: T.ink, fontWeight: 500 }}>{offer.name}</div>{offer.description && <p style={{ color: T.dim, lineHeight: 1.55 }}>{offer.description}</p>}{disclosure && <p style={{ color: T.dim, lineHeight: 1.55 }}>{disclosure.nature}</p>}</div>
          <div style={{ ...box, display: "grid", gridTemplateColumns: "1fr auto", gap: 20, alignItems: "end" }}><div><div style={mono}>PRICE</div><div style={{ color: T.dim, marginTop: 8 }}>{cadence(offer)} · future renewals can be cancelled</div></div><div style={{ fontSize: 30, color: T.ink, fontWeight: 600 }}>{money(offer.unitAmount, offer.currency)}</div></div>
          <div style={box}><div style={mono}>SELLER</div><p style={{ color: T.ink, lineHeight: 1.6 }}><strong>{OPERATOR.legalName}</strong><br />Org. no. {OPERATOR.orgNumber}<br />{OPERATOR.address}<br /><a className="link" href={`mailto:${OPERATOR.email}`}>{OPERATOR.email}</a></p><p style={{ color: T.dim, lineHeight: 1.55 }}>The company is not registered in the Norwegian VAT Register in this legal version. The amount shown above is the configured total recurring charge.</p></div>
          {disclosure && <div style={box}><div style={mono}>CANCEL / WITHDRAWAL</div><p style={{ color: T.dim, lineHeight: 1.6 }}>{disclosure.withdrawal}</p><p style={{ color: T.dim, lineHeight: 1.6 }}>{disclosure.cancellation}</p></div>}
          <div style={box}><label style={{ display: "flex", gap: 11, alignItems: "flex-start", color: T.ink, lineHeight: 1.55 }}><input type="checkbox" checked={accepted} onChange={(event) => setAccepted(event.target.checked)} style={{ marginTop: 4 }} /><span>I have reviewed the recurring price and <Link to="/legal/terms" className="link">terms</Link>, including <Link to="/legal/payments" className="link">payment, cancellation and withdrawal information</Link>. This is separate from any marketing consent.</span></label><button type="button" onClick={pay} disabled={!accepted || opening} style={{ marginTop: 20, border: `1px solid ${accepted ? T.ink : T.line}`, background: accepted ? T.ink : "transparent", color: accepted ? T.paper : T.faint, padding: "13px 17px", font: "inherit", fontWeight: 700, cursor: accepted ? "pointer" : "not-allowed" }}>{opening ? "OPENING STRIPE…" : "ORDER WITH OBLIGATION TO PAY"}</button><p style={{ color: T.faint, fontSize: 12.5, lineHeight: 1.55, marginTop: 14 }}>Stripe Checkout opens next. 4PLANET verifies payment server-side; a browser redirect alone is never accepted as proof of payment.</p></div>
        </div>}
      </Section>
    </PublicShell>
  );
