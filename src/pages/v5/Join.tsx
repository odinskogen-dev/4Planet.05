...
import { currentAttribution, trackProductEvent } from "@/lib/measurement";
...
export default function Join() {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const idempotencyKey = `join:${crypto.randomUUID()}`;
    const payload = {
      type: "4people",
    ...
    setState("sending");
    void trackProductEvent("signup_start", { attribution, properties: { form: "join" } });
    try {
        ...
        <div style={{ marginTop: "clamp(48px,7vw,80px)", display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: "clamp(28px,5vw,72px)", alignItems: "start" }} className="join-grid">
          <div>
            ...
            <p style={{ color: T.dim, fontSize: 15, lineHeight: 1.6, marginTop: 16, maxWidth: 520 }}>
              The form is wired to 4Planet&apos;s server-side registration endpoint. If the release environment has no approved persistent destination configured, the interface will say so and nothing will be stored or sent.
            </p>
          </div>
          ...
          {state === "delivered" ? (
            <div style={{ border: `1px solid ${T.lineStrong}`, padding: "clamp(22px,3vw,34px)" }}>
              ...
              <p style={{ marginTop: 14, fontSize: 16, lineHeight: 1.6 }}>Your registration was stored in the configured 4Planet interest system. This is a registration receipt, not membership, partnership or funding status.</p>
            </div>
          ) : state === "pending" ? (
            <div style={{ border: `1px solid ${T.lineStrong}`, padding: "clamp(22px,3vw,34px)" }}>
              <StatusLabel>DATA CAPTURE NOT ACTIVE</StatusLabel>
              <p style={{ marginTop: 14, fontSize: 16, lineHeight: 1.6 }}>The public form is ready, but this environment has no approved persistent destination enabled. Nothing was stored or sent.</p>
            </div>
          ) : (
            <form onSubmit={onSubmit} style={{ border: `1px solid ${T.lineStrong}`, padding: "clamp(22px,3vw,34px)", display: "grid", gap: 14 }}>
              ...
              <label style={{ display: "flex", alignItems: "flex-start", gap: 10, color: T.dim, fontSize: 13, lineHeight: 1.5 }}>
                ...
                <span>I agree that 4Planet may use these details to process my registration and contact me about this interest. This does not give separate permission for unrelated marketing. No account or payment is created.</span>
              </label>
