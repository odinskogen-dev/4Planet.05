...
import { actorBySlug } from "@/content/actorGold";
import {
  BERGEN_GRAPH,
...
const sourceById = (id: string) => LIVING_PLANET_SOURCES.find((source) => source.id === id);
...
export function BergenPlaceGold() {
  ...
  return <main className="lp-shell">
    ...
    <QuestionBlock code="02" question="WHAT DOES SCIENCE SAY?">
      <div className="lp-card-grid">{RESEARCH_OBJECTS.map((item) => <Link className="lp-card" key={item.id} to={`/research/${item.id.toLowerCase()}`}><span>{item.type.replaceAll("_", " ")} · {item.status}</span><h3>{item.title}</h3><p>{item.humanFinding}</p><b>WHAT WE KNOW →</b></Link>)}</div>
      <div className="lp-truth-note"><strong>TRUTH BY DESIGN</strong><p>Pro-Climate is ongoing. 4PLANET shows what the project studies, but does not manufacture “findings” the source has not published.</p></div>
    </QuestionBlock>

    <QuestionBlock code="03" question="WHY DOES IT MATTER?">
      <div className="lp-two"><article><span>FOR PEOPLE</span><h3>Planning rules affect everyday life.</h3><p>Accessibility, travel choices, time, cost, housing and neighbourhood quality can all be affected. The right choice is not identical for every person or district.</p></article><article><span>FOR THE LIVING PLANET</span><h3>Land and mobility choices create long-lived pressure patterns.</h3><p>The DNA cell connects those choices to evidence instead of treating climate, nature and human needs as separate worlds.</p></article></div>
    </QuestionBlock>

    <QuestionBlock code="04" question="WHO IS INVOLVED?">
      <div className="lp-actor-row"><Link to="/actors/bergen-kommune"><span>PUBLIC ACTOR</span><h3>Bergen kommune</h3><p>Decision / consultation authority</p></Link><Link to="/actors/institute-of-marine-research"><span>RESEARCH ACTOR</span><h3>Institute of Marine Research</h3><p>Research institution archetype</p></Link><Link to="/actors"><span>ACTOR INTELLIGENCE</span><h3>Explore the graph</h3><p>Monitoring · implementation · research · government · capital</p></Link></div>
    </QuestionBlock>
...
export function ResearchIndex() {
  return <main className="lp-shell"><Seo title="Research Intelligence — 4PLANET" description="Global Research Intelligence translated into living-planet relevance and human explanation." path="/research" robots="noindex,follow" /><LPNav /><header className="lp-hero lp-research-hero"><div className="lp-eyebrow"><span>GLOBAL RESEARCH INTELLIGENCE</span></div><h1>Research is useful when people can understand what it actually says.</h1><p>Source → findings or scope → limitations → place → problem → actors → decisions. Funding is context, never an automatic accusation.</p></header><section className="lp-research-list">{RESEARCH_OBJECTS.map((item) => <Link key={item.id} to={`/research/${item.id.toLowerCase()}`}><span>{item.status}</span><h2>{item.title}</h2><p>{item.whyItMatters}</p><b>OPEN HUMAN EXPLANATION →</b></Link>)}</section></main>;
}

export function ResearchDetail() {
  const { id } = useParams(); const item = researchById(id); if (!item) return <Navigate to="/research" replace />;
  return <main className="lp-shell"><Seo title={`${item.title} — Research | 4PLANET`} description={item.whyItMatters} path={`/research/${item.id.toLowerCase()}`} robots="noindex,follow" /><LPNav /><header className="lp-hero lp-research-detail"><div className="lp-eyebrow"><span>RESEARCH OBJECT</span><span>{item.status}</span></div><h1>{item.title}</h1><p>Research Intelligence does not replace the source. It makes scope, meaning and uncertainty legible.</p></header><div className="lp-research-questions"><article><span>01</span><h2>What did they find?</h2><p>{item.humanFinding}</p></article><article><span>02</span><h2>Why does it matter?</h2><p>{item.whyItMatters}</p></article><article><span>03</span><h2>How sure are we?</h2><p>{item.confidence}</p></article><article><span>04</span><h2>What didn’t they prove?</h2><p>{item.didNotProve}</p></article><article><span>05</span><h2>Who did the research?</h2><p>{item.researchers.length ? item.researchers.map((person) => `${person.name}${person.canonicalState === "UNRESOLVED" ? " · identity not yet resolved" : ""}`).join(" · ") : "No individual researchers asserted in this evidence object."}</p><p>{item.institutions.map((org) => org.name).join(" · ")}</p></article><article><span>06</span><h2>Who funded it?</h2><p>{item.funders.join(" · ")}</p></article><article><span>07</span><h2>Where does it matter?</h2><p>{item.placeIds.map((place) => place.replace("PLACE-", "")).join(" · ")}</p></article><article><span>08</span><h2>What could change because of it?</h2><p>Research can inform choices and decisions. 4PLANET does not claim a policy effect unless a specific Decision object cites or uses the evidence.</p></article></div><section className="lp-source-panel"><p>SOURCES</p><SourceLinks ids={item.sourceIds} /></section></main>;
}
