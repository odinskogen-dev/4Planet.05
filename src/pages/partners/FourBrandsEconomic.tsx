...
export default function FourBrandsEconomic() {
  ...
  async function importFile(file?: File|null) {
    ...
    try {
      const parsed=parseEconomicCsv(await file.text(),file.name);
      if(!parsed.length)throw new Error("No usable economic rows were found.");
      const parsedCash=cashInput.trim()===""?undefined:Number(cashInput.replace(/[^0-9.\-]/g,""));
      setRows(parsed); setConfirmedCash(Number.isFinite(parsedCash as number)?parsedCash:undefined);
      setDataLabel(`${file.name.toUpperCase()} · LOCAL SESSION ONLY`); setImportOpen(false); setImportError(""); setView("overview");
  ...
  function ask4Brands(event: FormEvent){
    ...
    const q=normalise(question); let response="";
    if(q.includes("margin")||q.includes("profit")) response=`Gross margin is ${twin.currentPeriod.grossMargin.toFixed(1)}%, ${pp(twin.grossMarginDeltaPp)} versus the previous 30 days. Recorded direct-cost intensity explains ${pp(twin.marginDrivers[0]?.value||0)}; the remainder stays unexplained until deeper price and mix evidence exists.`;
    else if(q.includes("cash")||q.includes("runway")){const low=[...twin.cash13Week].sort((a,b)=>a.endingCash-b.endingCash)[0];response=`Current ${twin.cashIsConfirmed?"confirmed":"recorded"} cash is ${money(twin.cash,twin.currency)}. The 13-week schedule reaches ${money(low?.endingCash||0,twin.currency)} in ${low?.label||"the forecast"}. This remains an estimate from dated imported AR, AP and renewals.`;}
    else if(q.includes("customer")){const best=twin.profitByCustomer[0];response=best?`${best.key} has the strongest recorded direct contribution at ${money(best.contribution,twin.currency)} (${best.contributionMargin.toFixed(1)}%). Allocated overhead is excluded.`:"Customer economics cannot be resolved from the current import.";}
    else if(q.includes("evidence")||q.includes("strongest")){const strong=twin.opportunities.find(item=>item.confidence==="HIGH")||twin.opportunities[0];response=strong?`${strong.title} has the strongest current evidence. Confidence: ${strong.confidence}. It uses ${strong.rowIds.length} exact imported row(s). Inspect the opportunity for calculation, assumptions and falsifier.`:"No opportunity currently has enough evidence.";}
    else {const top=twin.opportunities[0];response=top?`The largest detected value range is ${money(top.valueLow,twin.currency)}–${money(top.valueHigh,twin.currency)}: ${top.title}. This is ${top.truthClass.toLowerCase()}, not realised value.`:"No decision-grade value opportunity is currently detected.";}
    setAnswer(response);
  }
  ...
  if(!twin) return <PublicModel analysis={analysis} reset={()=>setAnalysis(null)} openImport={()=>setImportOpen(true)} loadDemo={loadDemo} importPanel={<ImportPanel open={importOpen} onClose={()=>setImportOpen(false)} cashInput={cashInput} setCashInput={setCashInput} importError={importError} onFile={importFile} onDemo={loadDemo}/>}/>;

  return <main className="fb-app fb-product">
    <header className="fb-topbar">
      <button className="fb-logo fb-logo-button" onClick={()=>{setRows([]);setDataLabel("")}}>4BRANDS<span>_</span></button>
      <button className="fb-company-chip" onClick={()=>{setRows([]);setDataLabel("")}}><i/><strong>{analysis.company.name}</strong><small>Company Twin</small></button>
      <div className="fb-top-actions"><span className="fb-session">{dataLabel}</span><button className="fb-ask" onClick={()=>setAskOpen(value=>!value)}>Ask 4BRANDS</button><button className="fb-add" onClick={()=>setImportOpen(true)}>+ Data</button></div>
    </header>
    <div className="fb-shell">
      ...
      <section className="fb-workspace">
        {dataLabel&&<div className="fb-mobile-session" aria-label="Data truth status">{dataLabel}</div>}
        ...
        {view==="money"&&<MoneyViewScreen twin={twin} mode={moneyView} setMode={setMoneyView}/>} 
        {view==="value"&&<ValueFinder twin={twin} active={activeOpportunity} setActive={setActiveOpportunity} onChoose={chooseOpportunity}/>} 
        {view==="decisions"&&<DecisionLedger twin={twin} selected={selectedOpportunity} state={decisionState} setState={setDecisionState} attribution={attribution} setAttribution={setAttribution} baseline={baseline} lockBaseline={lockBaseline} intervention={intervention} setIntervention={setIntervention} result={measuredResult} setResult={setMeasuredResult} advance={advanceDecision} onFind={()=>navigate("value")}/>} 
      </section>
    </div>
    ...
    {askOpen&&<AskPanel twin={twin} question={question} setQuestion={setQuestion} answer={answer} submit={ask4Brands} close={()=>setAskOpen(false)}/>} 
...
function FrontDoor({query,setQuery,runAnalysis,loading,loadingStep,error,clearError}:{query:string;setQuery:(v:string)=>void;runAnalysis:(event?:FormEvent,forced?:string)=>Promise<void>;loading:boolean;loadingStep:number;error:string;clearError:()=>void}){
  return <main className="fb-app fb-front">
    ...
    {error&&<div className="fb-toast"><strong>Research paused</strong><span>{error}</span><button onClick={clearError}>Close</button></div>}
  </main>;
}

function PublicModel({analysis,reset,openImport,loadDemo,importPanel}:{analysis:PublicAnalysis;reset:()=>void;openImport:()=>void;loadDemo:()=>void;importPanel:React.ReactNode}){
  return <main className="fb-app fb-public">
    <header className="fb-public-head"><button className="fb-logo fb-logo-button" onClick={reset}>4BRANDS<span>_</span></button><span>PUBLIC COMPANY MODEL</span><button onClick={reset}>Switch company</button></header>
