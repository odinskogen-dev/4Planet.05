...
export type FoodSourceLayer = {
  ...
  label: string;
  authority: string;
  state: "LIVE_API" | "EXISTING_ATLAS" | "OPEN_DATASET" | "ACCESS_GATED";
  role: string;
  limitation: string;
};
...
export const SAPIENS_CHAINS: SapiensChain[] = [
  { id: "food", label: "FOOD", humanNeed: "Eat", status: "GOLD_STANDARD", primaryMission: "food", pressureFamilies: ["land", "water", "nutrients", "climate", "biodiversity", "waste"], sourceFamilies: ["FAOSTAT", "Climate TRACE", "FAO GLW", "AQUASTAT", "GFW", "GBIF", "Global Fishing Watch", "UN Comtrade"] },
...
export const FOOD_SOURCES: FoodSourceLayer[] = [
  { id: "climate-trace-agriculture", label: "AGRICULTURE EMISSIONS SOURCES", authority: "Climate TRACE", state: "LIVE_API", role: "Geolocated agriculture-sector greenhouse-gas source records for the first live FOOD pressure layer.", limitation: "Inventory/model source records; not live plumes and not proof of ecological damage at a point." },
  { id: "faostat", label: "PRODUCTION + INPUTS + TRADE STATISTICS", authority: "FAO · FAOSTAT", state: "OPEN_DATASET", role: "Country/commodity production, inputs, food balances, emissions and other food-system statistics.", limitation: "Mostly statistical geography; not a farm-level traceability system." },
  { id: "glw4", label: "LIVESTOCK DISTRIBUTION", authority: "FAO · Gridded Livestock of the World v4", state: "OPEN_DATASET", role: "Spatial distribution and abundance surfaces for major livestock species.", limitation: "Modelled gridded estimates aligned to 2020; not current individual-animal positions." },
  { id: "aquastat", label: "WATER + AGRICULTURE", authority: "FAO · AQUASTAT", state: "OPEN_DATASET", role: "Water resources, use, irrigation and agricultural water management.", limitation: "Primarily country/statistical reporting; cadence differs by indicator." },
  { id: "gfw", label: "TREE COVER LOSS", authority: "Global Forest Watch · UMD", state: "EXISTING_ATLAS", role: "Spatial forest-loss context and alerts for land-pressure investigation.", limitation: "Tree-cover loss is not automatically deforestation and does not establish the commodity driver by itself." },
  { id: "nasa", label: "VEGETATION + RAIN + FIRE", authority: "NASA GIBS", state: "EXISTING_ATLAS", role: "NDVI, precipitation, true-colour imagery and thermal anomalies provide planetary context around production landscapes.", limitation: "Remote-sensing signals require source-specific interpretation; thermal anomaly is not automatically wildfire." },
  { id: "gbif", label: "SPECIES RECORDS", authority: "GBIF", state: "EXISTING_ATLAS", role: "Occurrence records connect food-production geographies to recorded life.", limitation: "Observation density is sampling effort as well as biodiversity; records are not population estimates." },
  { id: "comtrade", label: "COMMODITY TRADE FLOWS", authority: "UN Comtrade", state: "ACCESS_GATED", role: "Reporter-partner-commodity trade can connect production regions to consumption markets.", limitation: "Programmatic scale requires the current API access route; trade flow alone does not prove exact farm-to-buyer provenance." },
  { id: "gfw-fishing", label: "APPARENT FISHING EFFORT", authority: "Global Fishing Watch", state: "ACCESS_GATED", role: "AIS-derived apparent fishing activity can extend FOOD into seafood supply and marine pressure.", limitation: "API token required; AIS-derived apparent effort is not proof of illegal fishing and does not cover every vessel." },
];
