...
export function Analytics(){ const location=useLocation(); const ok=useMemo(allowed,[]); const [state,setState]=useState<Consent>(read);
  ...
  useEffect(()=>{ if(!ok||state!=="granted")return; consent("granted"); install(); window.gtag?.("event","page_view",{page_title:document.title,page_location:location.href,page_path:`${location.pathname}${location.search}`,content_group:"s4piens",site_host:host()}); },[ok,state,location.pathname,location.search]);
