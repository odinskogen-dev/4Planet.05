...
import { fourPlanetReturnUrl } from "./fourPlanetReturnUrl";
...
function IdentityModal({
  ...
  const google = async () => {
    ...
    const { error } = await client.auth.signInWithOAuth({
      provider: "google",
      options: { redirectTo: fourPlanetReturnUrl(window.location) },
    });
    if (error) {
    ...
    try {
      if (mode === "signup") {
        const result = await client.auth.signUp({
          email: email.trim(),
          password,
          options: { emailRedirectTo: fourPlanetReturnUrl(window.location) },
        });
