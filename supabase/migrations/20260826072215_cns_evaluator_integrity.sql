...
begin
  if old.state='RUNNING' and new.state in ('PASS','FAIL') and coalesce(current_setting('cns.evaluator_writer',true),'off')<>'on' then raise exception 'CNS_EVALUATION_STATE_REQUIRES_FINISH_FUNCTION'; end if;
  if old.state in ('PASS','FAIL','ERROR') and new.state is distinct from old.state then raise exception 'CNS_EVALUATION_FINAL_STATE_IMMUTABLE'; end if;
  return new;
end;
$$;
drop trigger if exists cns_evaluation_run_state_guard on cns.evaluation_runs;
create trigger cns_evaluation_run_state_guard before update on cns.evaluation_runs for each row execute function cns.guard_evaluation_run_state();
...
begin
  ...
  select count(*),count(*) filter(where critical=true),count(*) filter(where critical=true and passed=false)
    into v_total,v_critical,v_failed from cns.evaluation_assertions where evaluation_run_id=p_run;
  ...
  if cns.evaluation_enforcement_enabled() then
    ...
    select coalesce(jsonb_agg(req),'[]'::jsonb) into v_missing from jsonb_array_elements_text(v_contract.required_assertion_keys) req where not exists(select 1 from cns.evaluation_assertions a where a.evaluation_run_id=p_run and a.assertion_key=req);
  ...
  return v_state;
exception when others then perform set_config('cns.evaluator_writer','off',true); raise;
end;
$$;

create or replace view cns.v_evaluator_integrity_violations with (security_invoker=true) as
select 'PASS_EVAL_WITHOUT_ASSERTIONS'::text rule_id,'P0'::text severity,'EVALUATION'::text entity_type,r.evaluation_run_id::text entity_id,r.project_id,'Evaluation is PASS but has no assertions'::text summary
...
begin
  ...
  where i.state in ('OPEN','ACKNOWLEDGED') and not exists(select 1 from cns.v_superbrain_violations_v3 v where encode(digest(v.rule_id||'|'||v.entity_type||'|'||v.entity_id,'sha256'),'hex')=i.fingerprint);
  return v_count;
end;
$$;
