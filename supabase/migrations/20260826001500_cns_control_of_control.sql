...
begin
  select count(*),count(*) filter(where passed=false)
    into v_total,v_fail
  from cns.control_assertions where control_cycle_id=p_cycle;
  if v_total=0 then
    raise exception 'CNS_CONTROL_CYCLE_EMPTY';
  end if;
...
-- ---------------------------------------------------------------------------
-- LIBRARIAN INTEGRITY: memory cannot silently self-promote or be rewritten.
-- ---------------------------------------------------------------------------
create or replace function cns.guard_memory_integrity()
returns trigger
language plpgsql as $$
begin
  if tg_op='INSERT' then
    if new.state='ACTIVE' and coalesce(current_setting('cns.memory_writer',true),'off')<>'on' then
      raise exception 'CNS_MEMORY_ACTIVE_INSERT_FORBIDDEN';
    end if;
    return new;
  end if;

  if tg_op='DELETE' then
    if old.state='ACTIVE' then raise exception 'CNS_ACTIVE_MEMORY_DELETE_FORBIDDEN'; end if;
    return old;
  end if;
  ...
  if old.state='ACTIVE' then
    if new.content is distinct from old.content
      ...
      raise exception 'CNS_ACTIVE_MEMORY_CONTENT_IMMUTABLE';
    end if;
    if new.state is distinct from old.state and coalesce(current_setting('cns.memory_writer',true),'off')<>'on' then
      raise exception 'CNS_ACTIVE_MEMORY_STATE_CHANGE_REQUIRES_LIBRARIAN';
    end if;
  end if;

  if old.state='CANDIDATE' and new.state='ACTIVE'
     and coalesce(current_setting('cns.memory_writer',true),'off')<>'on' then
    raise exception 'CNS_MEMORY_PROMOTION_REQUIRES_LIBRARIAN';
  end if;
  return new;
end;
$$;
...
create trigger cns_memory_integrity_guard
before insert or update or delete on cns.memory_items
for each row execute function cns.guard_memory_integrity();
...
create or replace function cns.librarian_supersede_memory(p_old_memory_id text,p_new_memory_id text,p_event_id bigint)
returns boolean
language plpgsql security definer set search_path=cns,public as $$
declare v_new_state text;
begin
  select state into v_new_state from cns.memory_items where memory_id=p_new_memory_id;
...
-- ---------------------------------------------------------------------------
-- META-AUDITOR: independently checks Doctor/Librarian/context/source controls.
-- This view reports violations only; it does not repair truth.
-- ---------------------------------------------------------------------------
create or replace view cns.v_meta_control_violations with (security_invoker=true) as
select 'ACTIVE_MEMORY_WITHOUT_EVENT'::text rule_id,'P0'::text severity,'MEMORY'::text entity_type,
       m.memory_id::text entity_id,m.project_id,'Active memory has no promotion/writeback event'::text summary
from cns.memory_items m where m.state='ACTIVE' and m.last_event_id is null
union all
select 'ACTIVE_MEMORY_EXPIRED','P0','MEMORY',m.memory_id,m.project_id,'Expired memory remains ACTIVE'
from cns.memory_items m where m.state='ACTIVE' and m.valid_until is not null and m.valid_until<=now()
union all
select 'CONTEXT_BUDGET_BREACH','P0','CONTEXT',c.context_snapshot_id::text,c.project_id,'Compiled context exceeds declared fail-closed budget'
from cns.context_snapshots c where length(c.compiled_context::text) > c.token_budget*6
union all
select 'CONTEXT_SOURCE_REVISION_DRIFT','P0','CONTEXT',c.context_snapshot_id::text,c.project_id,'Valid context references an older source revision'
from cns.context_snapshots c
where c.invalidated_at is null
  and exists(
    select 1 from cns.source_registry s
    where s.state='ACTIVE' and s.current_revision is not null
      and coalesce(c.source_revisions->>s.source_id,'MISSING')<>s.current_revision
  )
union all
select 'DOCTOR_BLIND_SPOT','P0',v.entity_type,v.entity_id,v.project_id,'Doctor violation exists without an open matching health incident'
from cns.doctor_violations_v3 v
where not exists(
  select 1 from cns.health_incidents i
  where i.state in ('OPEN','ACKNOWLEDGED')
    and i.rule_id=v.rule_id and i.entity_type=v.entity_type and i.entity_id=v.entity_id
)
union all
select 'OPEN_CONFLICT_NOT_REFLECTED_IN_CLAIM_STATE','P1','CLAIM',c.claim_id,c.project_id,'Claim participates in open material conflict but knowledge_state is not CONFLICTED'
from cns.claims c
...
union all
select 'UNKNOWN_WITH_NUMERIC_CONFIDENCE','P1','CLAIM',c.claim_id,c.project_id,'UNKNOWN/INSUFFICIENT_EVIDENCE claim carries numeric confidence'
from cns.claims c where c.knowledge_state in ('UNKNOWN','INSUFFICIENT_EVIDENCE') and c.confidence is not null
union all
select 'ACCEPTED_LEARNING_WITHOUT_OUTCOME','P1','LEARNING',l.learning_id,l.project_id,'Accepted learning is not linked to an observed outcome'
from cns.learnings l where l.state='ACCEPTED' and l.outcome_id is null
union all
select 'ACCEPTED_LEARNING_WITHOUT_EVIDENCE','P1','LEARNING',l.learning_id,l.project_id,'Accepted learning has no evidence references'
from cns.learnings l where l.state='ACCEPTED' and jsonb_array_length(l.evidence_refs)=0
union all
select 'PUBLISHED_HYPOTHESIS_WITHOUT_FALSIFIER','P1','HYPOTHESIS',h.hypothesis_id,h.project_id,'Published hypothesis has no stated falsifier/counter-test'
from cns.hypotheses h where h.publication_state='PUBLISHED' and jsonb_array_length(h.falsifiers)=0;

create or replace function cns.audit_control_plane(p_label text default 'periodic',p_exact_sha text default null)
returns uuid
language plpgsql security definer set search_path=cns,public as $$
...
begin
  ...
  for r in select * from cns.v_meta_control_violations loop
    ...
    values(v_cycle,r.rule_id,r.severity,false,r.entity_type,r.entity_id,r.project_id,jsonb_build_object('summary',r.summary));
  end loop;

  perform cns.finish_control_cycle(v_cycle);
  return v_cycle;
end;
$$;
