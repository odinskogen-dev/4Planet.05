...
-- SUPERBRAIN AUTHORITY REPLAY GUARD
...
begin
  ...
  select * into b from cns.v_authority_routes where project_id=e.project_id and surface=p_surface and role='PRIMARY' limit 1;
  if not found then return false; end if;
...
begin
  ...
  if cns.authority_enforcement_enabled() then
    select * into v_event from cns.events e where e.project_id=p_project_id and e.event_type='PROJECT_STATE_COMMITTED' and cns.event_authorized_for_surface(e.event_id,'CURRENT_STATE') order by e.event_id desc limit 1;
  else
    select * into v_event from cns.events e where e.project_id=p_project_id and e.event_type='PROJECT_STATE_COMMITTED' order by e.event_id desc limit 1;
  end if;
...
from cns.events e
where cns.authority_enforcement_enabled() and e.event_type='PROJECT_STATE_COMMITTED' and not cns.event_authorized_for_surface(e.event_id,'CURRENT_STATE');
...
union all
select 'UNAUTHORIZED_STATE_EVENT','P0','EVENT',event_id::text,project_id,'PROJECT_STATE_COMMITTED event is not authorized by the current PRIMARY authority route'
