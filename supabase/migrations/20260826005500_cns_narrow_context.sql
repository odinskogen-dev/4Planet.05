-- SUPERBRAIN NARROW/JIT CONTEXT — 2026-08-26
...
begin
  ...
  -- Relevant source set only. A source that belongs to another project must not consume context,
  -- invalidate this context, or disclose its identity merely because it is globally ACTIVE.
...
-- Meta-Auditor drift rule must compare only sources actually included in a context snapshot.
...
from cns.memory_items m where m.state='ACTIVE' and m.valid_until is not null and m.valid_until<=clock_timestamp()
union all
...
union all
select 'CONTEXT_SOURCE_REVISION_DRIFT','P0','CONTEXT',c.context_snapshot_id::text,c.project_id,'Valid context references an older revision of a source it actually included'
from cns.context_snapshots c
where c.invalidated_at is null
  and exists(
    select 1 from jsonb_each_text(c.source_revisions) r(source_id,revision)
    join cns.source_registry s on s.source_id=r.source_id
    where coalesce(s.current_revision,'UNVERSIONED')<>r.revision or s.state<>'ACTIVE'
  )
union all
...
from cns.v_superbrain_violations_v2 v
where not exists(
  select 1 from cns.health_incidents i
  where i.state in ('OPEN','ACKNOWLEDGED') and i.rule_id=v.rule_id and i.entity_type=v.entity_type and i.entity_id=v.entity_id
)
union all
...
where c.state in ('ACTIVE','DISPUTED') and c.knowledge_state<>'CONFLICTED'
  and exists(select 1 from cns.conflict_claims cc join cns.conflicts cf using(conflict_id) where cc.claim_id=c.claim_id and cf.state='OPEN' and cf.severity in ('P0','P1'))
union all
...
union all
select 'RESTRICTED_GLOBAL_MEMORY','P0','MEMORY',m.memory_id,m.project_id,'Restricted/secret memory may not be global project-null context'
from cns.memory_items m where m.project_id is null and m.state='ACTIVE' and m.sensitivity_state in ('RESTRICTED','SECRET');
