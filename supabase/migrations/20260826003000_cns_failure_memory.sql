-- SUPERBRAIN FAILURE MEMORY + SOURCE IMPACT — 2026-08-26
...
create table if not exists cns.failure_recurrences (
  failure_recurrence_id bigint generated always as identity primary key,
  failure_id text not null references cns.failure_records(failure_id) on delete restrict,
  detected_at timestamptz not null default clock_timestamp(),
  evidence jsonb not null,
  event_id bigint references cns.events(event_id) on delete restrict
);

create or replace function cns.guard_failure_closure()
...
create trigger cns_failure_closure_guard before update on cns.failure_records
for each row execute function cns.guard_failure_closure();

create or replace function cns.record_failure_recurrence(p_failure_id text,p_evidence jsonb,p_event_id bigint default null)
returns bigint language plpgsql security definer set search_path=cns,public as $$
declare v_id bigint;
begin
  insert into cns.failure_recurrences(failure_id,evidence,event_id)
  values(p_failure_id,coalesce(p_evidence,'{}'::jsonb),p_event_id)
  returning failure_recurrence_id into v_id;
  update cns.failure_records
  set recurrence_count=recurrence_count+1,recurrence_last_at=clock_timestamp(),state='RECURRENT',updated_at=clock_timestamp()
  where failure_id=p_failure_id;
  if not found then raise exception 'CNS_FAILURE_NOT_FOUND'; end if;
  return v_id;
end;
$$;

-- Derive downstream epistemic impact from source state/freshness rather than forcing consumers to remember it.
create or replace view cns.v_source_health with (security_invoker=true) as
select s.source_id,s.name,s.state,s.current_revision,s.freshness_seconds,s.last_verified_at,
       case
         when s.state in ('INACTIVE','ARCHIVED','SUPERSEDED') then 'UNAVAILABLE'
         when s.state='DEGRADED' then 'DEGRADED'
         when s.state='ACTIVE' and s.freshness_seconds is not null and s.last_verified_at is null then 'UNVERIFIED'
         when s.state='ACTIVE' and s.freshness_seconds is not null and s.last_verified_at + make_interval(secs=>s.freshness_seconds)<=clock_timestamp() then 'STALE'
         when s.state='ACTIVE' then 'FRESH'
         else 'UNKNOWN'
       end as health_state
from cns.source_registry s;

create or replace view cns.v_source_dependents with (security_invoker=true) as
select c.source_id,'CLAIM'::text dependent_type,c.claim_id::text dependent_id,c.project_id,c.state::text dependent_state
from cns.claims c where c.source_id is not null and c.state in ('ACTIVE','DISPUTED')
union all
select e.source_id,'EVIDENCE',e.evidence_id,e.project_id,e.state
from cns.evidence e where e.source_id is not null and e.state in ('ACTIVE','DISPUTED')
union all
select o.source_id,'OBSERVATION',o.observation_id,o.project_id,o.state
from cns.observations o where o.source_id is not null and o.state in ('ACTIVE','DISPUTED')
union all
select sr.source_id,'CONTEXT',cs.context_snapshot_id::text,cs.project_id,
       case when cs.invalidated_at is null and cs.expires_at>clock_timestamp() then 'ACTIVE' else 'INACTIVE' end
from cns.context_snapshots cs
cross join lateral jsonb_object_keys(cs.source_revisions) sr(source_id)
where cs.invalidated_at is null and cs.expires_at>clock_timestamp();

create or replace view cns.v_source_impact with (security_invoker=true) as
select h.source_id,h.name,h.health_state,d.dependent_type,d.dependent_id,d.project_id,d.dependent_state
from cns.v_source_health h
join cns.v_source_dependents d using(source_id)
where h.health_state in ('UNAVAILABLE','DEGRADED','UNVERIFIED','STALE','UNKNOWN');

create or replace function cns.require_source_fresh(p_source_id text,p_allow_degraded boolean default false)
returns text language plpgsql security definer set search_path=cns,public as $$
declare v_state text;
begin
  select health_state into v_state from cns.v_source_health where source_id=p_source_id;
  if not found then raise exception 'CNS_SOURCE_NOT_FOUND'; end if;
  if v_state='FRESH' then return v_state; end if;
  if p_allow_degraded and v_state='DEGRADED' then return v_state; end if;
  raise exception 'CNS_SOURCE_NOT_FRESH:%',v_state;
end;
$$;

-- Extend Doctor without replacing the truth-model Doctor view.
create or replace view cns.v_operational_truth_violations with (security_invoker=true) as
select * from cns.doctor_violations_v3
union all
select 'SOURCE_IMPACT_ACTIVE','P1',i.dependent_type,i.dependent_id,i.project_id,
       'Active truth/context depends on a source that is not fresh: '||i.health_state
from cns.v_source_impact i where i.dependent_state='ACTIVE'
union all
select 'RECURRENT_VERIFIED_FAILURE','P0','FAILURE',f.failure_id,f.project_id,
       'A previously verified failure has recurred; prior prevention was insufficient'
...
begin
  ...
  from cns.v_operational_truth_violations
  ...
  where i.state in ('OPEN','ACKNOWLEDGED') and not exists(
    select 1 from cns.v_operational_truth_violations v where encode(digest(v.rule_id||'|'||v.entity_type||'|'||v.entity_id,'sha256'),'hex')=i.fingerprint
  );
  return v_count;
end;
$$;
...
('failure_memory','{"version":1,"verified":false,"state":"PENDING_CERTIFICATION","closure":"FIX+READBACK+REGRESSION_PASS+ROOT_CAUSE+CONTROL_GAP"}'::jsonb)
on conflict(key) do update set value=excluded.value,updated_at=clock_timestamp();

update cns.system_meta
set value='{"version":6,"migration":"20260826003000_cns_failure_memory","truth_model":"SUPERBRAIN_V4","control_of_control":"V1","failure_memory":"V1"}'::jsonb,
