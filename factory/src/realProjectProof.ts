...
function zeroLossForBoundedProof(currentTestSha: string, recoveryAuthority: string, sourcePath: string, checkedAt: string): ZeroLossEvidence {
  ...
  return {
    ...
    dispositions: [
      { donorRef: currentRef, feature: "exact current TEST implementation being changed by the bounded Factory proof", disposition: "ADOPT", evidence: [`Exact current TEST KING SHA ${currentTestSha}`, `Target ${sourcePath}`] },
      { donorRef: recoveryRef, feature: "historical/recovery value outside this bounded non-canonical proof change", disposition: "DEFER_WITH_REASON", reason: "This activation proof is not authorised to close or supersede the wider product Zero-Loss recovery. Historical value remains under the existing recovery authority.", evidence: [recoveryAuthority, "No Human Gold, Canon or product-wide Zero-Loss closure is inferred from this Factory candidate."] },
    ],
    orphanCount: 0,
    winnerParityEvidence: ["Bounded proof changes only the exact declared current TEST file.", "The wider recovery lane remains authoritative for historical product-value reconciliation.", "Candidate remains a draft TEST PR and cannot self-promote Human Gold or Canon."],
    checkedAt,
  };
}
...
function packageBase(id: string, projectId: string, title: string, targetPath: string, gapClosed: string, definitionOfDone: string[], requiredEvidence: string[], zeroLoss: ZeroLossEvidence, nowIso: string): WorkPackage {
  return { id, projectId, title, section: "PRODUCT_DESIGN", priority: "P0", goalLink: "FOUNDER-DIRECTED FACTORY FINAL AUTONOMOUS ACTIVATION SPRINT", gapClosed, deliverables: [`One bounded material improvement in ${targetPath}`, "Draft TEST candidate", "CI evidence", "390 mobile Browser QA evidence"], dependencies: [], writeScopes: [targetPath], definitionOfDone, requiredEvidence, zeroLoss, learningQuestion: "Can one shared Factory production contract improve materially different 4PLANET surfaces without flattening their product-specific character or truth boundaries?", createdAt: nowIso, estimatedValue: 8, criticalPath: 10, dependencyUnlock: 10, proofValue: 10, cashValue: 2, learningValue: 10, risk: 3, founderBurden: 1, concurrencyCost: 2, status: "READY" };
}
...
export function createRealProjectProofCases(currentTestSha: string, nowIso = new Date().toISOString()): RealProofCase[] {
  ...
  // Project ids are deliberately the exact registered product-authority keys.
