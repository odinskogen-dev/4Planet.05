...
export const onRequestPost = async (ctx: { request: Request; env: Env }): Promise<Response> => {
  ...
  form.set("line_items[0][price_data][product_data][description]", "Technical payment-path validation only. This is not the commercial fine-art print price.");
  ...
  form.set("custom_text[submit][message]", "TEST MODE — verifies Checkout and order confirmation only. No print is produced or shipped.");
  form.set("success_url", `${origin}/4market-stripe-test.html?checkout=success&session_id={CHECKOUT_SESSION_ID}`);
