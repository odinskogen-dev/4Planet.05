 ...
 * Honest contract: configured destination forwards; otherwise nothing is stored and the UI shows a direct-email fallback.
 */
...
export const onRequestPost = async (ctx: { request: Request; env: Env }): Promise<Response> => {
  ...
  const intendedAmountNok = typeof data.intendedAmountNok === "number" && Number.isInteger(data.intendedAmountNok) && data.intendedAmountNok >= 0 && data.intendedAmountNok <= 10_000_000 ? data.intendedAmountNok : null;
  const payload = {
    ...
    offerType: clean(data.offerType, 80),
    intendedAmountNok,
    referenceKey: clean(data.referenceKey, 160),
    consent: true,
  ...
  } catch { return json({ ok: false, error: "forward_error" }, 502); }
};
