...
function parseStripeSignature(header: string) {
  ...
  const signatures = values.filter((part) => part.startsWith("v1=")).map((part) => part.slice(3)).filter(Boolean);
...
export const onRequestPost = async (ctx: { request: Request; env: Env }): Promise<Response> => {
  ...
  if (env.MARKET_STRIPE_TEST_ENABLED !== "true") return json({ ok: false, error: "market_stripe_test_disabled" }, 503);

  const webhookSecret = env.STRIPE_MARKET_TEST_WEBHOOK_SECRET?.trim();
  if (!webhookSecret || !webhookSecret.startsWith("whsec_")) return json({ ok: false, error: "webhook_secret_missing" }, 503);

  const signatureHeader = request.headers.get("stripe-signature") ?? "";
  const rawBody = await request.text();
  if (!(await verifySignature(rawBody, signatureHeader, webhookSecret))) return json({ ok: false, error: "invalid_signature" }, 400);
  ...
  const isCompletionEvent = event.type === "checkout.session.completed" || event.type === "checkout.session.async_payment_succeeded";
  const paymentConfirmed = isCompletionEvent && session?.payment_status === "paid" && metadataOk && amountOk;
