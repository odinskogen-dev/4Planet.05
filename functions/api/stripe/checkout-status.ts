...
export const onRequestGet = async (ctx: { request: Request; env: Env }): Promise<Response> => {
  ...
  const metadataOk =
    ...
    ALLOWED_PRODUCT_KEYS.has(productKey) &&
    truthState === "TEST" &&
  ...
  const confirmed =
    ...
    session.id?.startsWith("cs_test_") === true &&
  ...
  return json({
    ...
    disclosure:
      productFamily === "IMPACT"
        ? "Payment confirmation is not partner delivery, ecological outcome or verified impact."
        : "Checkout confirmation establishes payment or subscription state only; product rights and benefits remain governed separately.",
  });
};
