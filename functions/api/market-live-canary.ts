...
const LIVE_SECRET_PREFIX = ["sk", "live", ""].join("_");
...
export const onRequestPost = async (ctx: { request: Request; env: Env }): Promise<Response> => {
  ...
  if (env.MARKET_STRIPE_LIVE_CANARY_ENABLED !== "true") return json({ ok: false, error: "live_canary_disabled" }, 503);

  const secret = env.STRIPE_LIVE_SECRET_KEY?.trim();
  if (!secret || !secret.startsWith(LIVE_SECRET_PREFIX)) return json({ ok: false, error: "stripe_live_secret_missing" }, 503);
  ...
  form.set("line_items[0][price_data][product_data][description]", "LIVE NOK 3 payment-path validation. This does not purchase a physical print and triggers no fulfilment.");
  form.set("line_items[0][quantity]", "1");
  form.set("customer_creation", "always");
  form.set("billing_address_collection", "auto");
  form.set("phone_number_collection[enabled]", "true");
  form.set("consent_collection[terms_of_service]", "required");
  form.set("custom_text[submit][message]", "LIVE PAYMENT CANARY — NOK 3. No print will be produced or shipped from this transaction.");
  form.set("success_url", `${origin}/4market-live-canary.html?checkout=success&session_id={CHECKOUT_SESSION_ID}`);
  ...
  if (!stripeResponse.ok || !stripePayload.url || !stripePayload.id?.startsWith("cs_live_") || stripePayload.livemode !== true) {
    return json({ ok: false, error: "live_checkout_create_failed", stripeType: stripePayload.error?.type ?? null }, 502);
  }

  return json({ ok: true, mode: "live_canary", amountNok: 3, productId: PRODUCT_ID, sessionId: stripePayload.id, url: stripePayload.url });
};
