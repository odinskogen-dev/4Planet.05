 ...
 * Official Norwegian reference spine backed by Matvaretabellen / Mattilsynet.
 * Reference composition evidence only; never a silent barcode/SKU substitute.
 */
...
function json(body: unknown, status = 200): Response {
  return new Response(JSON.stringify(body), { status, headers: { "content-type": "application/json; charset=utf-8", "cache-control": "public, max-age=900, s-maxage=21600, stale-while-revalidate=86400", "x-content-type-options": "nosniff" } });
}
...
function relevance(item: JsonObject, query: string): number {
  ...
  return words.length && words.every((word) => name.includes(word)) ? 250 + words.length : -1;
}
...
export const onRequestGet = async ({ request }: PagesContext): Promise<Response> => {
  ...
  try {
    ...
    if (!Array.isArray(payload)) return json({ ok: false, kind: "source_error", evidenceState: "UNAVAILABLE", retrievedAt, source: { id: "matvaretabellen", adapterVersion: ADAPTER_VERSION, endpoint: FOODS_ENDPOINT }, message: "Unexpected Matvaretabellen payload." }, 502);
    ...
    return json({ ok: false, kind: "source_error", evidenceState: "UNAVAILABLE", retrievedAt, source: { id: "matvaretabellen", adapterVersion: ADAPTER_VERSION, endpoint: FOODS_ENDPOINT }, message: error instanceof Error ? error.message : "Matvaretabellen request failed" }, 503);
  }
};
